...
the_joke = "Why did the chicken cross the road"
def speak(command):
    engine = pyx3()
    engine.say(command)
    engine.runAndWait()
def commands(mt):
    ...
    if "stop" in mt or "continue" in mt:
        ...
        if "text" in mt:
            for element in str(mt.split("text")[1]).strip():
                pig(element)
        else:
            for element in str(mt.split("line")[1]).strip():
                pig(element)
            pig("enter")
    elif "time" in mt:
        ...
        speak("Opening google")
        wopen("https://www.google.com")
        for element in "explain "+the_joke:
            pig(element)
        pig("enter")
        return
    elif "flip a coin" in mt:
        result_flip = int(randint(0,1))
        if result_flip == 0:
            speak("Head")
        else:
            speak("Tail")
        return
    elif "system exit" in mt or "systemexit" in mt or "system accent" in mt:
        speak("Good Bye!")
        quit()
    ...
    while (1):
        try:
            with mic() as source2:
                ...
                mt = str(r.recognize_google(audio2)).lower()
                if "imagine" in mt:
                    flag = not flag
